"""
enrich_vol5_fishbase_gbif.py
-----------------------------
Đồng bộ dữ liệu Sinh học (Biology), Hình thái EN (Morphology EN), Sinh thái EN (Ecology EN),
Giá trị kinh tế EN (Economic Value EN), và IUCN Status từ FishBase + GBIF cho Tập 5 (Cá biển Việt Nam - Tập V).
...
Chạy:
  python3 scripts/enrich_vol5_fishbase_gbif.py
"""
...
def get_gbif_data(sci_name: str, accepted_name: str = None):
    """Lấy IUCN status và descriptions từ GBIF theo tên khoa học hoặc accepted name"""
    for name in [sci_name, accepted_name]:
        if not name:
            continue
...
def main():
    print("=" * 70)
    print("ENRICHMENT FISHBASE + GBIF CHO TẬP V (279 loài)")
    print("=" * 70)

    sp_map, eco_map, rep_map, name_index = build_fishbase_lookups()

    # 1. Fetch species from Vol 5 Supabase
    print("\n[2] Lấy danh sách 279 loài Tập 5 từ Supabase...")
    req = urllib.request.Request(
        f"{SUPABASE_URL}/rest/v1/species?collection_id=eq.ca-bien&volume=eq.5&select=*&order=species_index",
    ...
    for idx, sp in enumerate(species_list, 1):
        ...
        accepted_name = sp.get("worms_accepted_name") or ""

        # Check FishBase index: try original name first, then accepted name
        parts = sci_name.strip().split()
        key = f"{parts[0].lower()} {parts[1].lower()}" if len(parts) >= 2 else ""
        spec_code = name_index.get(key)

        if not spec_code and accepted_name:
            acc_parts = accepted_name.strip().split()
            acc_key = f"{acc_parts[0].lower()} {acc_parts[1].lower()}" if len(acc_parts) >= 2 else ""
            spec_code = name_index.get(acc_key)
        ...
        iucn_status, gbif_desc = get_gbif_data(sci_name, accepted_name)
        ...
        time.sleep(0.08) # Respectful to GBIF API

    print("\n" + "=" * 70)
    print(f"HOÀN TẤT ENRICHMENT TẬP 5:")
